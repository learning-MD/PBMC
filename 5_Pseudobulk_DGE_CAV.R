...
y <- DGEList(counts = counts[,2:41], genes = counts$gene, group = metadata$group)
y$samples
y
...
write_csv(df, "PBMC_CellRanger_6.1.2/Upload/DGE/CAV_v_nonCAV/Platelets.csv")
...
# Limma
# Comparing CAV vs non-CAV
y <- DGEList(counts = counts[,2:39], genes = counts$gene, group = metadata$group)
y$samples
y
